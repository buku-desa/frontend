...
export default function BukuCard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error(err)
        setError('Gagal memuat buku lembaran desa')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleView = (doc: Document) => {
    ...
    setSelectedPDF({
      url: pdfUrl,
      title: doc.tentang, // tampilkan nama dokumen
    })
  }
  ...
  if (loading) return <p className="text-gray-500">Memuat buku lembaran desa...</p>
